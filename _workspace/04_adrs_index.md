...
## Per-decision acceptance evidence pointer

- ADR-001 OKF — `crates/okf` property tests + strict-write linter +
  permissive-read accepts unknown `type`/keys.
...
- ADR-003 Git — `crates/haven-git` integration tests `safe_existing_vault.rs`,
  `expected_hash_replace.rs`, `symlink_confinement.rs`.
- ADR-004 Local runtime — `crates/haven-engine` tests `loopback_only.rs`,
  `cloud_preflight.rs`, plus `crates/haven-index tests/embedding_versioning.rs`.
- ADR-005 Sync — `crates/haven-sync` two-device + three-device tests +
  rotation/replay harnesses.
- ADR-006 Memory — `crates/haven-memory` tests `file_backed_retain.rs` and
  `consolidation_review.rs`; numeric recall numbers in
  `docs/research/longmem-eval-bakeoff.md`.
- ADR-007 Local model — `crates/haven-engine` tests `model_manifest_pin.rs`
  and `embedding_stamp.rs`; per-tier benchmark in
  `docs/research/hardware-benchmarks.md`.
